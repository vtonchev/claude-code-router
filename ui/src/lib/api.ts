...
class ApiClient {
  ...
  // DELETE request (no Content-Type header to avoid Fastify 400 error)
  async delete<T>(endpoint: string): Promise<T> {
    const url = `${this.baseUrl}${endpoint}`;
    const headers: Record<string, string> = {
      'Accept': 'application/json',
    };
    if (this.tempApiKey) headers['X-Temp-API-Key'] = this.tempApiKey;
  ...
  async getLogs(filePath: string): Promise<string[]> {
    return this.getLogsRaw(filePath);
  }

  // Private raw fetch for text streams
  private async getLogsRaw(filePath: string): Promise<string[]> {
    const url = `${this.baseUrl}/logs?file=${encodeURIComponent(filePath)}`;
    const headers: Record<string, string> = {};
    if (this.tempApiKey) headers['X-Temp-API-Key'] = this.tempApiKey;
    else if (this.apiKey) headers['X-API-Key'] = this.apiKey;
  ...
  async clearAllLogs(): Promise<void> {
    return this.delete<void>('/logs?all=true');
  }
}
