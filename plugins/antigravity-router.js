...
module.exports = async function router(req, config, context) {
  ...
  console.log(
    `[custom-router] Incoming request, tools: ${JSON.stringify(toolTypes)}`
  );
  ...
  if (hasWebSearch) {
    console.log(
      `[custom-router] Web search detected → antigravity-websearch,gemini-2.5-flash`
    );
  ...
  console.log(
    `[custom-router] Model "${originalModel}" → antigravity,${antigravityModel}`
  );
