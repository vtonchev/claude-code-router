...
const readConfigFile = async (filePath: string) => {
  try {
    ...
    return null; // Return null if file does not exist or read fails
  }
};
...
const getProjectSpecificRouter = async (req: any) => {
  ...
  if (req.sessionId) {
    ...
    if (project) {
      ...
      // Try to read sessionConfig file first
  ...
  return undefined; // Return undefined to indicate using original configuration
};
...
// In-memory cache, mapping sessionId to project name
// null value indicates searched before but not found
// Use LRU cache, limit 1000 entries
...
export const searchProjectBySession = async (
  ...
  try {
    ...
    // Concurrently check if sessionId.jsonl exists in each project folder
    ...
    // Cache not found result (null value indicates searched before but not found)
